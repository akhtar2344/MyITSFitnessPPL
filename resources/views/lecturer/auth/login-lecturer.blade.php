...
<head>
  ...
  <title>Lecturer Login | myITS Fitness</title>
...
<body class="bg-white text-slate-800">
  <div class="min-h-screen grid grid-cols-1 lg:grid-cols-2">
    ...
    {{-- Kiri: ilustrasi (lebih turun, rapat ke bawah) --}}
      ...
      <div class="w-full max-w-sm space-y-6">
        ...
        <div class="pt-2">
          <h2 class="text-2xl font-extrabold text-slate-900">
            Hi, <span class="text-[#2d6df6]">Verifier</span>
          </h2>
        </div>

        {{-- Form --}}
        <form action="{{ route('lecturer.login.process') }}" method="POST" class="space-y-4">
          @csrf
          <div>
        ...
        {{-- Login with MyITS --}}
        <a
          href="{{ route('lecturer.login.myits') }}"
