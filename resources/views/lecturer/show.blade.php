...
<head>
  ...
  <title>{{ $student['name'] ?? 'Student Detail' }} | myITS Fitness</title>
  ...
  <header class="h-16 bg-white border-b">
    <div class="max-w-7xl mx-auto h-full px-6 flex items-center justify-between">
      <div class="flex items-center gap-3">
        <a href="#">
          <img src="{{ asset('images/myitsfitness-logo.png') }}" alt="myITS Fitness" class="h-7 w-auto">
        </a>
      </div>
    ...
    <div class="max-w-7xl mx-auto px-6 py-8">
      <div class="grid grid-cols-12 gap-8">
        ...
        <aside class="col-span-12 md:col-span-3">
          <nav class="space-y-3">
            <a href="#" class="flex items-center gap-3 rounded-xl border bg-white px-4 py-3 shadow-sm hover:bg-slate-50">
              <img src="{{ asset('images/icon-home.png') }}" class="w-6 h-6" alt="">
              <span class="font-medium">Dashboard</span>
            </a>
            <a href="#" class="flex items-center gap-3 rounded-xl border bg-white px-4 py-3 shadow-sm ring-1 ring-blue-100">
        ...
        <section class="col-span-12 md:col-span-9">
          ...
          <h1 class="text-4xl font-extrabold tracking-tight">{{ $student['name'] ?? 'Unknown Student' }}</h1>
              ...
              <div class="p-6">
                ...
                <div class="flex flex-wrap items-center justify-between gap-4">
                  <div class="flex items-center gap-4">
                    <img 
                      src="{{ asset($student['avatar'] ?? 'images/icon-user.png') }}" 
                  ...
                  <div class="flex items-center gap-2.5">
                    <button class="inline-flex items-center justify-center h-11 px-6 rounded-2xl bg-emerald-500 text-white font-semibold shadow-sm hover:bg-emerald-600 active:translate-y-px">Accept</button>
                    <button class="inline-flex items-center justify-center h-11 px-6 rounded-2xl bg-rose-500 text-white font-semibold shadow-sm hover:bg-rose-600 active:translate-y-px">Reject</button>
                    <button class="inline-flex items-center justify-center h-11 px-6 rounded-2xl border-2 border-amber-400 text-amber-600 font-semibold bg-white hover:bg-amber-50 active:translate-y-px">Request Revision</button>
                  </div>
                </div>
                ...
                <div class="mt-6 grid grid-cols-2 gap-y-3 text-sm md:text-base">
                  ...
                  <div class="font-medium">{{ $student['submitted'] ?? '—' }}</div>

                  <div class="text-slate-500">Activity:</div>
                  <div class="font-medium">{{ $student['activity'] ?? '—' }}</div>

                  <div class="text-slate-500">Location:</div>
                  <div class="font-medium">{{ $student['location'] ?? '—' }}</div>

                  <div class="text-slate-500">Duration:</div>
                  <div class="font-medium">{{ $student['duration'] ?? '—' }}</div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <img 
                      src="{{ asset($student['proof'] ?? 'images/placeholder-proof.png') }}" 
                      alt="{{ $student['name'] ?? 'Proof' }}" 
                  ...
                  <div>
                    ...
                    <div class="max-w-sm w-full h-44 md:h-48 lg:h-52 rounded-xl border bg-slate-100/70 flex items-center justify-center text-slate-400">—</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <aside class="col-span-12 xl:col-span-4 rounded-2xl bg-white border shadow-sm p-6 h-full flex flex-col">
              ...
              <div class="mt-4 space-y-4 flex-1 overflow-y-auto pr-1">
                <div class="flex items-start gap-3">
                  <img src="{{ asset('images/heru susanto.png') }}" alt="{{ $student['reviewer'] ?? 'Reviewer' }}" class="w-9 h-9 rounded-full object-cover border shadow-sm shrink-0"/>
                  <div>
                    <div class="font-medium">{{ $student['reviewer'] ?? '—' }}</div>
                    <div class="text-slate-600 text-sm">{{ $student['comment'] ?? 'No comments yet.' }}</div>
                  </div>
                </div>
              </div>
              ...
              <div class="pt-4 mt-4 border-t">
                <form id="commentForm" class="flex items-center gap-3">
                  <input id="commentInput" type="text" placeholder="Type your comment" class="flex-1 min-w-0 rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100">
                  <button id="sendBtn" type="submit" class="send-btn px-4 py-2 rounded-lg bg-[#7b61ff] text-white font-medium shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">Send</button>
                </form>
              </div>
            </aside>
          </div>
